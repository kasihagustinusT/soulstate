...
When a selector runs (via `useStore` or `computed`), SoulState executes it using a **Zero-Allocation Tracking Proxy**. This proxy intercepts property accesses and records which keys your selector depends on.
