...
```typescript
const storeA = createStore({ state: { a: 1 } });
const storeB = createStore({ state: { b: 2 } });
...
try {
  ...
  transaction.rollback(); // Reverts to state before transaction
} catch (error) {
...
```typescript
import { enableInstrumentation } from 'soulstate';

enableInstrumentation();

// Access metrics
const metrics = store.getMetrics();
console.log('Flush duration:', metrics.flushDuration);
console.log('Selector runs:', metrics.selectorRuns);
```
