...
SoulState's architecture is built on three systems-grade pillars: **Invalidation Graph Propagation**, **Deterministic Scheduling**, and **Zero-Allocation Tracking**. This document details how these systems interact to provide a high-performance granular reactive runtime.
...
4.  **PropagationEngine**: A level-based dispatch system that ensures glitch-free updates in deterministic topological order.
5.  **SubscriptionManager**: Optimized dual linked-list management for global and granular subscribers.
6.  **Scheduler**: A microtask-based deterministic batching engine.
...
### Surgical Tracking
When a component or computed value accesses a state key, it is registered as a dependent in the graph. SoulState uses a **Zero-Allocation Tracking Proxy** (in `src/internals/invalidation.ts`) to detect these dependencies with minimal GC overhead.
