...
By reusing a single Proxy instance and a single `Set` for key tracking, SoulState achieves **Zero-Allocation Tracking** during the propagation flush, which is critical for maintaining high Hertz in 100,000+ subscriber scenarios.
