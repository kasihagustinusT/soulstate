import { CodeBlock } from "../../components/CodeBlock";

# Getting Started

Welcome to **SoulState**!  
This guide will help you build your first store using the ultra-fast SoulState runtime.

---

## 1. Installation

Install SoulState and React:

<CodeBlock language="bash">
{`npm install soulstate react`}
</CodeBlock>

---

## 2. Create a Store

Make a file called `store.ts`:

<CodeBlock language="typescript">
{`import { createStore } from 'soulstate';

export interface CounterState {
  count: number;
  increment: () => void;
  decrement: () => void;
}

export const useCounterStore = createStore<CounterState>((set) => ({
  count: 0,
  increment: () => set((state) => ({ count: state.count + 1 })),
  decrement: () => set((state) => ({ count: state.count - 1 })),
}));
`}
</CodeBlock>

---

## 3. Use it in your Component

Now use SoulState’s `useStore` selector-driven hook:

<CodeBlock language="tsx">
{`import { useStore } from 'soulstate';
import { useCounterStore, CounterState } from './store';

const selectCount = (state: CounterState) => state.count;
const selectActions = (state: CounterState) => ({
  increment: state.increment,
  decrement: state.decrement,
});

export function Counter() {
  const count = useStore(useCounterStore, selectCount);
  const { increment, decrement } = useStore(useCounterStore, selectActions);

  return (
    <div>
      <h1>Count: {count}</h1>
      <button onClick={increment}>+1</button>
      <button onClick={decrement}>-1</button>
    </div>
  );
}
`}
</CodeBlock>

---

Selectors ensure this component re-renders **only when the selected slice changes**, giving you **optimal performance with zero overhead**.
